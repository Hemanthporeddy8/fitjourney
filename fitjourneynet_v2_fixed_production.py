...
Expected training time: ~7-8 hours on T4 GPU (40 Epochs)
Expected final PCKh@0.5: >85% (MediaPipe gets ~88%)
"""
import gc
import torch.backends.cudnn as cudnn
cudnn.benchmark = True # Speeds up training on fixed-size inputs

# ══════════════════════════════════════════════════════════
# CELL 1 — Install dependencies
# ══════════════════════════════════════════════════════════
# !pip install pycocotools albumentations onnx onnxruntime -q
...
BATCH_SIZE  = 64        # Reduced for single GPU stability (64 is plenty)
EPOCHS      = 40        # 40 Epochs on full COCO is enough for MobileNet-sized models
LR          = 1e-3
WORKERS     = 4         # Increased for faster data loading on 1 GPU
CHECKPOINT_PATH = "/kaggle/working/fitjourney_v2_checkpoint.pth"
BEST_MODEL_PATH = "/kaggle/working/fitjourney_net_v2_best.pth"
...
class COCOKeypointDataset(Dataset):
    ...
    def __init__(self, root, split="train", transform=None, min_keypoints=5):
        ...
        # CRITICAL: Delete the giant raw JSON data to save System RAM
        del data
        gc.collect()
...
class DepthwiseSeparable(nn.Module):
    def __init__(self, in_ch, out_ch, stride=1, use_se=True):
        super().__init__()
        self.use_skip = (stride == 1 and in_ch == out_ch)
        mid_ch = in_ch * 3 # Expansion factor
        ...
        if use_se:
            # Squeeze-and-Excitation (adds tiny bit of params but huge accuracy boost)
    ...
    def forward(self, x):
        out = self.conv[0:3](x) # DW + BN + HS
        if self.se: out = out * self.se(out)
        out = self.conv[3:5](out) # PW + BN
        if self.use_skip: return x + out
        return out

class FitJourneyBackbone(nn.Module):
    """
    100% Custom IP — Scratch-built Backbone.
    Optimized for high-accuracy pose estimation without external weights.
    """
    def __init__(self):
        super().__init__()
        self.stem = nn.Sequential(
            nn.Conv2d(3, 24, 3, stride=2, padding=1, bias=False), # 128x128
            nn.BatchNorm2d(24),
            nn.Hardswish(inplace=True),
        )
        # Stage 1: 64x64, 48ch
    ...
    def forward(self, x):
        ...
        s1 = self.stage1(x) # 64x64
        s2 = self.stage2(s1) # 32x32
        s3 = self.stage3(s2) # 16x16
        s4 = self.stage4(s3) # 8x8
        return s1, s2, s3, s4
...
class FitJourneyDecoder(nn.Module):
    ...
    def __init__(self, num_kp=17):
        ...
        self.regress_head = nn.Sequential(
            ...
            # No Sigmoid here — we use logits for training stability with AMP
        )

    def forward(self, s1, s2, s3, s4):
        # Top-down pathway
        p4 = self.lat4(s4)                                           # 192,8,8
        p3 = self.lat3(s3) + F.interpolate(p4, size=s3.shape[-2:],  # 192,16,16
                                           mode='bilinear', align_corners=False)
        p3 = self.ref3(p3)

        p2 = self.lat2(s2) + F.interpolate(p3, size=s2.shape[-2:],  # 192,32,32
                                           mode='bilinear', align_corners=False)
        p2 = self.ref2(p2)

        p1 = self.lat1(s1) + F.interpolate(p2, size=s1.shape[-2:],  # 192,64,64
                                           mode='bilinear', align_corners=False)
        p1 = self.ref1(p1)
...
class FitJourneyNetV2(nn.Module):
    ...
    def forward(self, x, return_regression=False):
        ...
        if return_regression:
            # During training, we return raw coords (logits)
            # but during inference/export, we want 0-1 range
...
class RegressionLoss(nn.Module):
    ...
    def forward(self, pred_coords, target_joints, target_weight):
        # pred_coords:   (B, 17, 3) — predicted x, y, conf (logits)
        # target_joints: (B, 17, 2)
        # target_weight: (B, 17, 1)

        tgt = target_joints / INPUT_SIZE
        w   = target_weight

        # Apply sigmoid ONLY to xy for the L1 loss calculation
        pred_xy = torch.sigmoid(pred_coords[:,:,:2])
        xy_loss = F.smooth_l1_loss(pred_xy * w, tgt * w)

        # Visibility/confidence loss using LOGITS (AMP safe)
        vis = (target_weight.squeeze(-1) > 0).float()  # (B,17)
        conf_loss = F.binary_cross_entropy_with_logits(pred_coords[:,:,2], vis)
...
def train_epoch(model, loader, optimizer, criterion, device, epoch, scaler):
    ...
    for batch_idx, (images, heatmaps, weights) in enumerate(loader):
        ...
        # AMP Mixed Precision (Using modern torch.amp interface)
...
def main():
    ...
    print(f"Using single GPU: {DEVICE} (More stable for custom IP models)")
    ...
    scaler     = torch.amp.GradScaler('cuda') # Modern AMP scaler
    
    # Fast 'Scratch' Scheduler
    ...
    # RESUME LOGIC (Preserves your IP and hard-earned progress)
    if os.path.exists(CHECKPOINT_PATH):
        ...
        print(f"Loading previous best model weights: {BEST_MODEL_PATH}")
        # Allows resuming even if the full checkpoint was lost, to save time
        state = torch.load(BEST_MODEL_PATH)
        m = model.module if hasattr(model, 'module') else model
        m.load_state_dict(state)
        # Note: start_epoch remains 1 if no checkpoint, but starting with better weights is faster

    for epoch in range(start_epoch, EPOCHS + 1):
        t0 = time.time()

        train_loss = train_epoch(model, train_loader, optimizer, criterion, DEVICE, epoch, scaler)
